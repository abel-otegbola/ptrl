...
export default function ProductCard({ product }: { product: { id: string, title: string, img: string, price: string, available: boolean } }) {
    const ref1 = useRef<HTMLDivElement>(null)
    const isVisible = useIsVisible(ref1);

    return (
        <div ref={ref1} className={`flex flex-col items-center gap-2 mb-4 overflow-hidden`}>
            <Link href={`/product/${product.id}`} className={`flex items-center justify-center relative text-center w-full 2xl:h-[350px] xl:h-[300px] sm:h-[270px] h-[240px] bg-[#f6f6f4] rounded-lg duration-700 ease-in-out ${isVisible ? "translate-y-[0%] opacity-[1]" : "opacity-[0] translate-y-[10%]"}`} style={{ transitionDelay: `${+product.id * 100}ms` }}>
                <Image src={product?.img} alt={product?.title} width={2400} height={2400} className="md:h-auto h-auto w-full" />
                {
                    product.available ?
                    ""
                    :
