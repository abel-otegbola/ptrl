...
export default function ProductCard({ product }: { product: { id: string, title: string, img: string, price: string } }) {
    ...
    return (
        <div ref={ref1} className={`flex flex-col items-center gap-2 mb-4 overflow-hidden`}>
            ...
            <Link to={`/product/${product.id}`} className={`text-center uppercase font-bold md:leading-[24px] leading-[16px] md:text-[15px] text-[12px] duration-700 ${isVisible ? "translate-y-[0%] opacity-[1]" : "opacity-[0] translate-y-[-40%]"}`}>{product.title}</Link>
