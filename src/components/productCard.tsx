...
export default function ProductCard({ product }: { product: { id: string, title: string, img: string, price: string } }) {
    ...
    return (
        <div ref={ref1} className={`flex flex-col items-center gap-2 mb-4 overflow-hidden`}>
            ...
            <Link to={`/product/${product.id}`} className={`text-center uppercase font-bold leading-[24px] text-[14px] duration-700 ${isVisible ? "translate-y-[0%] opacity-[1]" : "opacity-[0] translate-y-[-40%]"}`}>{product.title}</Link>
