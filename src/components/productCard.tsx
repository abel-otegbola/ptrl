...
export default function ProductCard({ product }: { product: { id: string, title: string, img: string, price: string } }) {
    ...
    return (
        <div ref={ref1} className={`flex flex-col items-center gap-2 mb-4 overflow-hidden`}>
            <Link href={`/product/${product.id}`} className={`flex items-center justify-center text-center w-full 2xl:h-[350px] xl:h-[300px] sm:h-[270px] h-[240px] bg-[#f6f6f4] rounded-lg duration-700 ease-in-out ${isVisible ? "translate-y-[0%] opacity-[1]" : "opacity-[0] translate-y-[10%]"}`} style={{ transitionDelay: `${+product.id * 100}ms` }}>
                <Image src={product?.img} alt={product?.title} width={260} height={260} className="md:h-[260px] md:w-[250px] h-[200px] w-[200px]" />
            </Link>
