...
export default function Dropdown({ className, disabled, label, name, options, value, onChange, error, placeholder }: dropdownProps) {
    ...
    return (
        <div ref={optionsRef} className={`relative flex flex-col gap-1 ${className}`}>
            ...
            <div className={`rounded-[8px] absolute bottom-[70%] left-0 w-full max-h-[200px] z-[1000] bg-white shadow-md duration-700 overflow-y-auto border border-gray-500/[0.2] ${(focus || useSearch) ? "block" : "hidden"}`}>
